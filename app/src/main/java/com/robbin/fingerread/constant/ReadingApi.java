...
public class ReadingApi {
    ...
    public static String getBookInfo(int position,BookBean book){
        switch (position){
            case 0:
                if(book.getSummary().isEmpty()) break;
                return book.getSummary();
            case 1:
                if(book.getCatalog().isEmpty())break;
                return  book.getCatalog();
            case 2:
                if(book.getAuthor_intro().isEmpty())break;
                return book.getAuthor_intro();
        }
        return FingerReadApplication.AppContext.getString(R.string.text_noinfo);
    }
}
